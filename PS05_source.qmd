...
ggplot(hate_crimes, aes(x = trump_support, y = hate_crimes, color = trump_support)) +
  geom_boxplot(fill = "grey91") +
  labs(x = "Level of Trump Support", y = "Hate Crimes per 100 K individuals", title = "Hate crimes by Trump support level", color = "Trump Support") +
  theme_bw()
```
:::
...
-   As the level of `trump_support` increases, the number of `hate_crimes` seems to generally decrease. This may be expected as the people in these areas would have been happier since the candidate they supported won the election. After talking about it in class, I realized its more likely that areas that have low Trump support are urban areas, with more cultural a diversity. With more varying cultures in one area, it may be more likely for hate crimes happen between these groups.
:::
...
kable(get_regression_table(hate_mod))
```
:::
...
-   The intercept in this regression table is the mean amount of hate crimes per 100,000 people in a state with high Trump support.
:::
...
-   The model estimates that the number of hate crimes per 100,000 people in a state with "low" Trump support is around `r round(predict(hate_mod, newdata = data.frame(trump_support = "low")), digits = 4)`.

```{r}
# Type your code and comments inside the code chunk
predict(hate_mod, newdata = data.frame(trump_support = "low"))
```
:::
...
```{r}
# Type your code and comments inside the code chunk
max(c(predict(hate_mod, newdata = data.frame(trump_support = "low")),
predict(hate_mod, newdata = data.frame(trump_support = "high"))))
```

-   The model estimates that hate crimes are more frequent in states that show "low". 
:::
...
```{r, label = "delta"}
# Type your code and comments inside the code chunk
predict(hate_mod, newdata = data.frame(trump_support = "medium")) -> a
predict(hate_mod, newdata = data.frame(trump_support = "high")) -> b
a - b
```

-   The amount of hate crimes in "medium" Trump-support states were `r a -b` hate crimes per 100,00 people higher compared to those in "high" Trump-support states.
:::
...
-   One possible fitted value for $\hat{y}$ is 0.460.

-   Another possible fitted value for $\hat{y}$ is 0.222.

-   The third possible fitted value for $\hat{y}$ is 0.191.
:::
...
-   These numbers are the same as the three possible fitted values $\hat{y}$ for this model.
:::
...
$\hat{y}=0.191 + 0.2691111(0) + 0.0314(0)$

```{r}
# Type your code and comments inside the code chunk
(0.191 + 0.2691111*(0) + 0.0314*(0))
```

-   When a state has Trump support that is "high", the regression equations gives us 0.191 hate crimes per 100,000 people.
:::
...
# Type your code and comments inside the code chunk
hate_crimes |>
  select(hate_crimes, state, trump_support) |>
...
-   The five states with the highest rate of hate crimes are `r  question11$state[1:4]`, and `r question11$state[5]`. These results do surprise me because I thought there would be more hate crime like this in areas with the larger cities like New York and California. I thought New York and California would also be in the top 5.
:::
...
```{r}
# Type your code and comments inside the code chunk
ggplot(hate_crimes, aes(x = unemployment, y = hate_crimes, color = unemployment)) +
  geom_boxplot(fill = "grey91") +
  labs(x = "Level of Unemployment", y = "Hate Crimes per 100 K individuals", title = "Hate crimes by Unemployment level", color = "Unemployment Level") +
  theme_bw()
```
:::
...
kable(get_regression_table(job_mod))
```
:::
...
$\widehat{\text{Hate Crimes}} = 0.289 + 0.031 * (1_{\text{low unemployment}})$
:::
...
-   The intercept is the mean amount of hate crimes per 100,000 people when the unemployment level is high. 
:::
...
```{r}
# Type your code and comments inside the code chunk
predict(job_mod, newdata = data.frame(unemployment = "low"))
```

-   The model estimates the number of hate crimes per 100,000 people in a state with "low" unemployment to be around `r round(predict(job_mod, newdata = data.frame(unemployment = "low")), digits = 4)`.
:::
...
# Type your code and comments inside the code chunk
predict(job_mod, newdata = data.frame(unemployment = "low"))
predict(job_mod, newdata = data.frame(unemployment = "high"))
```

-   The two possible fitted values $\hat{y}$ for this model are `r round(predict(job_mod, newdata = data.frame(unemployment = "low")), digits = 4)` and `r round(predict(job_mod, newdata = data.frame(unemployment = "high")), digits = 4)`. There are only two of these compared to three because the character vector `unemployment` is only measured with the characters `low` and `high` instead of `low`, `medium`, and `high` like `trump_support`.
:::
...
-   Residuals are found through subtracting the actual number of hate crimes by the prediction `hate_crimes_hat`.
:::
...
kable(get_regression_table(job_med_mod))
summary(job_med_mod)
```
:::
...
-   There were more crimes in areas with a high median household income. The difference between states with "low" and "high" levels of household income is around 0.1604 hate crimes per 100,000 people.
:::
...
```{r}
# Type your code and comments inside the code chunk
kable(head(get_regression_points(job_med_mod)))
```
:::
...
hate_crimes |>
  filter(state == "Maine") |>
  select(state, hate_crimes, median_house_inc)
get_regression_points(job_med_mod) |>
  filter(ID == 2)
```

-   Based on the data, the model has underpredicted the level of hate crime in Maine based on its median household income.
:::
